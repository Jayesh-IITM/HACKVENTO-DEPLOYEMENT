...
def _build_sqlalchemy_database_uri() -> str:
    """Build SQLAlchemy DB URI from environment.

    Supports Railway-style variables for MySQL and Postgres:
    - DATABASE_URL (preferred)
    - MySQL:  MYSQLHOST, MYSQLPORT, MYSQLUSER, MYSQLPASSWORD, MYSQLDATABASE
    - Postgres: PGHOST, PGPORT, PGUSER, PGPASSWORD, PGDATABASE
    - Generic: DB_HOST, DB_PORT, DB_USER, DB_PASSWORD, DB_NAME (+ optional DB_ENGINE)
    """

    # 1) Railway/Heroku-style full URL
    database_url = os.getenv('DATABASE_URL')
    if database_url:
        # Ensure SQLAlchemy uses pymysql for MySQL URLs
        if database_url.startswith('mysql://'):
            return database_url.replace('mysql://', 'mysql+pymysql://', 1)
        return database_url

    # 2) Railway Postgres plugin variables
    pg_host = os.getenv('PGHOST') or os.getenv('POSTGRES_HOST')
    pg_port = os.getenv('PGPORT') or os.getenv('POSTGRES_PORT') or os.getenv('DB_PORT')
    pg_user = os.getenv('PGUSER') or os.getenv('POSTGRES_USER') or os.getenv('DB_USER')
    pg_password = os.getenv('PGPASSWORD') or os.getenv('POSTGRES_PASSWORD') or os.getenv('DB_PASSWORD')
    pg_db = os.getenv('PGDATABASE') or os.getenv('POSTGRES_DB') or os.getenv('DB_NAME')
    db_engine = (os.getenv('DB_ENGINE') or '').lower()

    if pg_host and pg_user and pg_password and pg_db and (db_engine in ('postgres', 'postgresql') or (pg_port and str(pg_port) == '5432')):
        return f"postgresql+psycopg2://{pg_user}:{pg_password}@{pg_host}:{pg_port}/{pg_db}"

    # 3) Railway MySQL plugin variables
...
# Build database URI safely
try:
    db_uri = _build_sqlalchemy_database_uri()
except Exception as e:
    print(f"⚠️  Warning: Database config error: {e}")
...
# CORS configuration: allow calls from Railway frontend only (or as configured)
cors_origins_env = os.getenv('CORS_ORIGINS', '').strip()
if cors_origins_env:
    allowed_origins = [o.strip() for o in cors_origins_env.split(',') if o.strip()]
else:
    # Safe defaults: allow localhost for dev and your Railway frontend in prod
    allowed_origins = [
        'http://localhost:5173',
        'http://127.0.0.1:5173',
        'http://localhost:3000',
        'https://hackvento-deployement-production.up.railway.app',
    ]
...
# Note: db.create_all() removed - tables will be created on first DB access
# This allows the app to start even if DB is misconfigured

# Firebase initialization toggle
...
def register():
    try:
        ...
        if USE_FIREBASE:
            existing = fb.get_user_by_email(data['email'])
            if existing:
                return jsonify({'error': 'Email already registered'}), 400

            pwd_hash = generate_password_hash(data['password'])
            user_id = fb.create_user(data['email'], pwd_hash, data['role_id'], is_verified=False)
            ...
            if data['role_id'] == 1:
                ...
                fb.create_student(user_id, student_data)
            elif data['role_id'] == 2:
                company_data = {
                ...
                fb.create_company(user_id, company_data)

            user_doc = fb.get_user(user_id)
            return jsonify({'message': 'Registration successful. Please wait for admin verification.', 'user': user_doc}), 201
        ...
        if data['role_id'] == 1:
            ...
            db.session.flush()
            verification = StudentVerification(student_id=student.id, status='Pending')
        ...
        db.session.commit()
        return jsonify({'message': 'Registration successful. Please wait for admin verification.', 'user': user.to_dict()}), 201
...
def login():
    try:
        ...
        if USE_FIREBASE:
            user = fb.get_user_by_email(data['email'])
            if not user or not check_password_hash(user.get('password_hash', ''), data['password']):
                return jsonify({'error': 'Invalid email or password'}), 401
            if not user.get('is_verified', False) and user.get('role_id') != 3:
                return jsonify({'error': 'Account not verified by admin'}), 403

            access_token = create_access_token(identity=str(user['id']))
            profile = None
            if user.get('role_id') == 1:
                profile = fb.get_student_by_user_id(user['id'])
            elif user.get('role_id') == 2:
                profile = fb.get_company_by_user_id(user['id'])
            return jsonify({'access_token': access_token, 'user': user, 'profile': profile}), 200
        ...
        return jsonify({'access_token': access_token, 'user': user.to_dict(), 'profile': profile}), 200
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def student_profile():
    try:
        ...
        if USE_FIREBASE:
            student = fb.get_student_by_user_id(str(user_id))
            if not student:
            ...
            sid = student['id']
            update = {
                'full_name': data.get('full_name', student.get('full_name')),
                'phone': data.get('phone', student.get('phone')),
                'skills': data.get('skills', student.get('skills')),
                'resume_url': data.get('resume_url', student.get('resume_url')),
                'tenth_percentage': data.get('tenth_percentage', student.get('tenth_percentage')),
                'twelfth_percentage': data.get('twelfth_percentage', student.get('twelfth_percentage')),
                'experience': data.get('experience', student.get('experience')),
                'projects': data.get('projects', student.get('projects')),
                'certifications': data.get('certifications', student.get('certifications')),
                'linkedin_url': data.get('linkedin_url', student.get('linkedin_url')),
                'github_url': data.get('github_url', student.get('github_url'))
            }
            req_fields = [update.get('full_name'), update.get('phone'), student.get('cgpa'), update.get('skills')]
            if all(req_fields):
                update['profile_completed'] = True
            fb.update_student(sid, update)
            refreshed = fb.get_student_by_user_id(str(user_id))
...
def root():
    ...
    return jsonify({'status': 'Backend is running', 'api': '/api'}), 200
